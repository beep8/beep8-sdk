...
int main(int argc, char *argv[]){
  ...
  try {
    ...
    size_t pos_slash = _png_filename.find_last_of("/");
    std::string filename_with_ext = _png_filename.substr(pos_slash + 1);

    size_t pos_dot = filename_with_ext.find_last_of(".");
    std::string filename = filename_with_ext.substr(0, pos_dot);
