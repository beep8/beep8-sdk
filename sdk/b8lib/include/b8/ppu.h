...
/**
 * @brief Allocates and initializes a flush command in the PPU command buffer.
 * 
 * This function allocates memory for a `b8PpuFlush` structure within the PPU command buffer.
 * The flush command forces the PPU to commit any cached changes to VRAM or palette to the actual hardware memory.
 * 
 * **Note:** Frequent use of this API can impact performance due to the overhead of flushing. It is recommended 
 * to limit flush operations by optimizing command ordering and only using the flush command when necessary.
 * 
 * @param cmd_ Pointer to the `b8PpuCmd` structure representing the PPU command buffer.
 * @return Pointer to the newly allocated `b8PpuFlush` structure.
 */
extern b8PpuFlush* b8PpuFlushAlloc(b8PpuCmd* cmd_);

/**
 * @brief Allocates and initializes a flush command in the PPU command buffer with a specified Z-order.
 * 
 * This function allocates memory for a `b8PpuFlush` structure within the PPU command buffer and sets the Z-order for rendering.
 * The flush command forces the PPU to commit any cached changes to VRAM or palette to the actual hardware memory.
 * 
 * **Note:** Frequent use of this API can impact performance due to the overhead of flushing. It is recommended 
 * to minimize flush operations and set Z-order carefully to optimize rendering and cache efficiency.
 * 
 * @param cmd_ Pointer to the `b8PpuCmd` structure representing the PPU command buffer.
 * @param otz_ Z-order value for the flush operation, determining its rendering order.
 * @return Pointer to the newly allocated `b8PpuFlush` structure.
 */
